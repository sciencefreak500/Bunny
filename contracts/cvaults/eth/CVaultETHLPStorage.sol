...
contract CVaultETHLPStorage is CVaultETHLPState, PausableUpgradeable {
    ...
    uint128 public constant LEVERAGE_MIN = 5e17;  // 50%
    ...
    modifier increaseNonceOnlyRelayers(address lp, address _account, uint112 nonce) {
        require(msg.sender == address(relayer), "LPVault: not a relayer");
        require(accountOf(lp, _account).nonce == nonce, "LPVault: invalid nonce");
        _;
        increaseNonce(lp, _account);
    }

    modifier onlyStateFarming(address lp) {
        require(stateOf(lp, msg.sender) == State.Farming, "LPVault: not farming state");
        _;
    }

    modifier validLeverage(uint128 leverage) {
        require(LEVERAGE_MIN <= leverage && leverage <= LEVERAGE_MAX, "LPVault: leverage range should be [50%-150%]");
        _;
    }

    modifier notPausedPool(address lp) {
        require(_pools[lp].paused == false, "LPVault: paused pool");
        _;
    }
    ...
    function resetAccountExceptNonceAndState(address lp, address _account) private {
        ...
        _pools[lp].accounts[_account] = Account(0, 0, 0, 0, account.nonce, 0, 0, address(0), account.state);
    }
    ...
    function convertState(address lp, address _account, State state) internal {
        ...
        if (state == State.Idle) {
            require(msg.sender == address(relayer), "LPVault: only relayer can resolve emergency state");
            require(currentState == State.Withdrawing || currentState == State.Liquidating || currentState == State.EmergencyExited,
                "LPVault: can't convert to Idle"
            );
            ...
            require(currentState == State.Idle || currentState == State.Farming,
                "LPVault: can't convert to Depositing");
            ...
            require(currentState == State.Depositing || currentState == State.UpdatingLeverage,
                "LPVault: can't convert to Farming");
        } else if (state == State.Withdrawing) {
            require(currentState == State.Farming,
                "LPVault: can't convert to Withdrawing");
        } else if (state == State.UpdatingLeverage) {
            require(currentState == State.Farming,
                "LPVault: can't convert to UpdatingLeverage");
        } else if (state == State.Liquidating) {
            require(currentState == State.Farming,
                "LPVault: can't convert to Liquidating"
            );
        } else if (state == State.EmergencyExited) {
            require(_account == msg.sender, "LPVault: msg.sender is not the owner of account");
            require(currentState == State.Depositing || currentState == State.Withdrawing || currentState == State.UpdatingLeverage, "LPVault: unavailable state to emergency exit");
            require(account.updatedAt + EMERGENCY_EXIT_TIMELOCK < block.timestamp, "LPVault: timelocked");
        } else {
